...
<body>
  ...
  <?php
    $errMsg = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        if (strpos($errMsg, 'login_tutor.php?1') !== false){
            echo "<div style='text-align:center; font-size:2rem; color:#f69'>You have successful SignUp</div>";
        }

    ?>
          <div class="login-container d-flex align-items-center justify-content-center">
            <form class="login-form text center" action="logincode_tutor.php" method="post">
              ...
              <div>
                <p class="mb-3">Don't have an account? <a href="signup_tutor.php">SignUp</a></p>
              </div>
            </form>
      </div>
    </div>
